...
export class Post_Middlewares {
  ...
  async http_Out_And_Cookie(parentSpanInst, bh) {
    const spanInst = this.tracerService.createSpan(
      'http_Out_And_Cookie',
      parentSpanInst
    );
    try {
      this.tracerService.sendData(spanInst, bh);
      await this.cookie_out(bh, parentSpanInst);
      //appendnew_next_http_Out_And_Cookie
      return bh;
    } catch (e) {
      return await this.errorHandler(
        bh,
  ...
  async cookie_out(bh, parentSpanInst) {
    try {
      bh = this.sdService.cookieSetter(bh, bh.local.cookie_data);

      bh.web.res.status(200).send(bh.local.responces);

      return bh;
    } catch (e) {
      return await this.errorHandler(bh, e, 'sd_leItYu7OSd66r3nz');
    }
  }
